...
@section('subtitle')
    Buat penawaran harga untuk pelanggan.
@endsection
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div class="lg:col-span-2 space-y-5">
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div class="form-group">
                            ...
                            <input type="text" name="quotation_number" class="form-input" value="{{ old('quotation_number') }}" required>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div class="form-group">
                            ...
                            <input type="date" name="quotation_date" class="form-input" value="{{ old('quotation_date', date('Y-m-d')) }}" required>
                        ...
                        <div class="space-y-3">
                            <template x-for="(item, index) in items" :key="index">
                                <div class="flex gap-3 items-start p-3 bg-gray-50 rounded-xl">
                                    ...
                                    <div class="flex-[2]">
                                        <input type="text" x-model="item.description" :name="`items[${index}][description]`" placeholder="Deskripsi" class="form-input text-sm">
                                    </div>
                ...
                <div class="space-y-5">
                    ...
                    <div class="flex gap-3 pt-2">
                        ...
                        <a href="{{ route('quotations.index') }}" class="btn-ghost">Batal</a>
                    </div>
                </div>
            </div>
        </form>
...
<script>
    function quotationForm() {
        return {
            ...
            addItem() { this.items.push({ product_id: '', description: '', quantity: 1, price: 0 }); },
            removeItem(index) { if (this.items.length > 1) { this.items.splice(index, 1); this.calcTotal(); } },
            calcTotal() { this.subtotal = this.items.reduce((s, i) => s + (parseInt(i.quantity) || 0) * (parseFloat(i.price) || 0), 0); this.total = this.subtotal + (parseFloat(this.tax) || 0); },
