        ...
        <form method="POST" action="{{ route('purchase-orders.store') }}" x-data="poForm()">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div class="lg:col-span-2 space-y-5">
                    <div>
                        <h3 class="text-sm font-semibold text-gray-700 mb-3">Informasi PO</h3>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-4">
                            <div class="form-group">
                                <label class="form-label">PR (opsional)</label>
                                <select name="purchase_request_id" class="form-input">
                                    <option value="">Pilih PR</option>
                                    @foreach($purchaseRequests as $pr)
                                    <option value="{{ $pr->id }}" {{ old('purchase_request_id') == $pr->id ? 'selected' : '' }}>{{ $pr->request_number }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="form-group">
                ...
                <div class="space-y-5">
                        ...
                        <div class="card-body !p-4">
                            ...
                            <div class="space-y-2">
                                <div class="flex justify-between text-sm">
                                    ...
                                    <span class="font-medium text-gray-800" x-text="items.length"></span>
                                </div>
                    ...
                    <div class="flex gap-3 pt-2">
                        <button type="submit" class="btn-primary flex-1">Buat PO</button>
                        <a href="{{ route('purchase-orders.index') }}" class="btn-ghost">Batal</a>
                    </div>
                </div>
            </div>
        </form>
