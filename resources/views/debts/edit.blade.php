...
<div class="flex items-center gap-3 mb-5">
    ...
    <div>
        <h2 class="text-lg font-bold text-gray-800">Edit Hutang/Piutang</h2>
        <p class="text-sm text-gray-400">Ubah data transaksi</p>
    </div>
</div>
        ...
        <form method="POST" action="{{ route('debts.update', $debt) }}">
            ...
            <div class="form-group mb-6">
                ...
                <div class="grid grid-cols-2 gap-3 mt-1">
                    <label class="flex items-center gap-3 p-4 border border-gray-200 rounded-xl cursor-pointer has-[:checked]:border-emerald-400 has-[:checked]:bg-emerald-50/50 transition-all hover:border-gray-300">
                        <input type="radio" name="type" value="receivable" {{ ($debt->type === 'receivable' || old('type') === 'receivable') ? 'checked' : '' }} class="sr-only" required>
                    ...
                    <label class="flex items-center gap-3 p-4 border border-gray-200 rounded-xl cursor-pointer has-[:checked]:border-red-400 has-[:checked]:bg-red-50/50 transition-all hover:border-gray-300">
                        <input type="radio" name="type" value="payable" {{ ($debt->type === 'payable' || old('type') === 'payable') ? 'checked' : '' }} class="sr-only" required>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                <div class="form-group">
                    ...
                    <input type="text" name="contact_name" value="{{ old('contact_name', $debt->contact_name) }}" required class="form-input" placeholder="Nama orang/perusahaan">
                    @error('contact_name') <p class="form-error">{{ $message }}</p> @enderror
                </div>
                <div class="form-group">
                    <label class="form-label">Jumlah</label>
                    <input type="number" name="amount" value="{{ old('amount', $debt->amount) }}" required min="0" class="form-input" placeholder="Rp 0">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                <div class="form-group">
                    ...
                    <textarea name="description" rows="2" class="form-input" placeholder="Keterangan...">{{ old('description', $debt->description) }}</textarea>
                    @error('description') <p class="form-error">{{ $message }}</p> @enderror
                </div>
                <div class="form-group">
                    <label class="form-label">Jatuh Tempo</label>
                    <input type="date" name="due_date" value="{{ old('due_date', $debt->due_date->format('Y-m-d')) }}" required class="form-input">
