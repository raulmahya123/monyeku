...
@section('subtitle', 'Tambahkan pengguna baru ke perusahaan')
...
<div class="flex items-center gap-3 mb-6">
    ...
    <div>
        <h1 class="text-lg font-semibold text-gray-900">Tambah Pengguna</h1>
        <p class="text-sm text-gray-500">Tambahkan pengguna baru ke perusahaan</p>
    </div>
</div>
...
<div class="max-w-2xl">
    <div class="card">
        ...
        <div class="card-body">
            <form method="POST" action="{{ route('users.store') }}" x-data="{ selectedRole: '{{ old('role', 'staff') }}' }">
                @csrf

                <div class="grid grid-cols-2 gap-5 mb-5">
                    <div class="form-group">
                        <label class="form-label">Nama</label>
                        <input type="text" name="name" value="{{ old('name') }}" required class="form-input" placeholder="Nama lengkap">
                ...
                <div class="grid grid-cols-2 gap-5 mb-5">
                    <div class="form-group">
                        ...
                        <input type="password" name="password" required min="8" class="form-input" placeholder="Min. 8 karakter">
                ...
                <div class="mb-5">
                    ...
                    <p class="form-hint mt-2">Owner hanya bisa ditetapkan saat pembuatan perusahaan.</p>
                </div>

                {{-- Multi Company Assignment --}}
                <div class="mb-5">
                    <label class="form-label">Akses Perusahaan</label>
                    <p class="form-hint mb-3">Pilih perusahaan yang akan diakses oleh pengguna ini.</p>
                    @if($companies->count() > 0)
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                        @foreach($companies as $cmp)
                        <label class="flex items-center gap-3 p-3.5 border border-gray-200 rounded-xl cursor-pointer transition-all hover:border-orange-300 hover:bg-orange-50/30 has-[:checked]:border-orange-400 has-[:checked]:bg-orange-50 has-[:checked]:ring-2 has-[:checked]:ring-orange-200">
                            <input type="checkbox" name="company_ids[]" value="{{ $cmp->id }}"
                                {{ in_array($cmp->id, old('company_ids', [Auth::user()->current_company_id])) ? 'checked' : '' }}
                                class="w-4 h-4 rounded border-gray-300 text-orange-500 focus:ring-orange-400 shrink-0">
                            <div class="w-8 h-8 rounded-lg bg-gray-100 flex items-center justify-center text-xs font-bold text-gray-600 shrink-0">
                                {{ substr($cmp->name, 0, 1) }}
                            </div>
                            <div class="min-w-0">
                                <p class="text-sm font-medium text-gray-800 truncate">{{ $cmp->name }}</p>
                                <p class="text-xs text-gray-400">{{ $cmp->pivot->role ?? 'admin/staff' }}</p>
                            </div>
                        </label>
                        @endforeach
                    </div>
