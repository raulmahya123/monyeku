...
@section('title', 'Transaksi Berulang Baru')
...
<div class="flex items-center gap-3 mb-6">
    ...
    <div>
        <h1 class="text-lg font-semibold text-gray-900">Transaksi Berulang Baru</h1>
        <p class="text-sm text-gray-500">Buat transaksi otomatis yang berjalan periodik</p>
    </div>
</div>
        ...
        <form method="POST" action="{{ route('recurring.store') }}">
            ...
            <div class="mb-5">
                ...
                <div class="grid grid-cols-2 gap-3">
                    <label class="flex items-center gap-3 p-4 border-2 rounded-xl cursor-pointer transition-all {{ old('type') === 'income' ? 'border-emerald-500 bg-emerald-50' : 'border-gray-200 hover:border-emerald-300 hover:bg-emerald-50/50' }}">
                        <input type="radio" name="type" value="income" {{ old('type') === 'income' ? 'checked' : '' }} class="text-emerald-600 focus:ring-emerald-500" required>
                        <div>
                            <span class="text-sm font-medium text-gray-700">Pemasukan</span>
                            <p class="text-xs text-gray-400">Pendapatan berulang</p>
                        </div>
                    </label>
                    <label class="flex items-center gap-3 p-4 border-2 rounded-xl cursor-pointer transition-all {{ old('type') === 'expense' ? 'border-red-500 bg-red-50' : 'border-gray-200 hover:border-red-300 hover:bg-red-50/50' }}">
                        <input type="radio" name="type" value="expense" {{ old('type') === 'expense' ? 'checked' : '' }} class="text-red-600 focus:ring-red-500" required>
                        <div>
            ...
            <div class="mb-5">
                ...
                <textarea name="description" rows="2"
                    ...
                    placeholder="e.g. Sewa gedung bulanan">{{ old('description') }}</textarea>
